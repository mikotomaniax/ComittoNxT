package jp.dip.muracoro.comittont;
...
public class FileSelectActivity extends Activity implements OnTouchListener, ListNoticeListener, BookmarkListenerInterface, Handler.Callback {
	...
	protected void onNewIntent(Intent intent) {
		...
		// TODO: ここでサーバーIndexを設定しておかないと更新されないタイミングがあるのかも
	...
	private void moveFileSelect(int serverindex, boolean history) {
		...
		Log.d("server","moveFlieSelect " + mServer.getSmbVersion(serverindex));
		mServerSelect = serverindex;
	...
	private boolean moveFileSelectFromServer(int svrindex, String path) {
		...
		Log.d("server","moveFlieSelectFromServer " + mServer.getSmbVersion(svrindex));
		mServerSelect = svrindex;
	...
	public boolean handleMessage(Message msg) {
		...
		// }
			...
			case DEF.HMSG_LOADFILELIST: {
				loadListViewAfter();
				//mServerは現在選択中のサーバー情報のコピー
				//mServer.setSmbVersion(msg.arg2);
				// msg.arg2にてSMBバージョンを受け取っている
				// これをサーバー情報に反映させる
				if( mServerSelect > -1 ){
					ServerSelect server = new ServerSelect(mSharedPreferences, this);
					if (mServerSelect != ServerSelect.INDEX_LOCAL) {
						server.select( mServerSelect );
						server.setSmbVersion( msg.arg2 );
						server.save( mServerSelect );
					}
				}
				break;
			}
