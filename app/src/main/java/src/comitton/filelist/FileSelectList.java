...
import src.comitton.common.SmbFileAccess;
...
//		WDFileData[] wdfiles = null;
		...
		try {
			if (mListMode == LISTMODE_LOCAL) {
                ...
                SmbTreeHandle th = sfile.getTreeHandle();
				if(th.isSMB2()){
					Log.i("jcifs-ng","SMB2+");
					mSmb = 2;
				}else {
                ...
                sfile.getTreeHandle().close();
                th = null;
                //*/
                //SmbFileAccess sfa = new SmbFileAccess();
                //sfilelite = sfa.getSmbFileList( mUri + mPath, mUser, mPass );
                //sfa = null;
                sfiles = sfile.listFiles();
				if (sfiles == null) {
					flag = true;
				}
                //sfilelite = sfile.list();
                //if( sfilelite == null ){
                //    flag = true;
                //}
			} else {
			...
			for (int i = 0; i < length; i++) {
				if (mListMode == LISTMODE_LOCAL) {
					...
					if (name != null && len >= 1 && name.substring(len - 1).equals("/")) {
						flag = true;
						size = 0;       // SMB2接続では匿名アクセスが禁止されているためか、権限の無いフォルダ情報等にアクセス出来ないので、取得しないように変更
						try{
							date = sfiles[i].lastModified();
						}
						catch (SmbException e){
							// SMB2接続では匿名アクセスが禁止されているためか、権限の無いフォルダ情報等にアクセス出来ないので
							date = 0;
						}
						date = 0;
					} else {
