...
public class ServerSelect {
	...
	public ServerSelect(SharedPreferences sharedPreferences, Context context) {
		...
		for (int i = 0 ; i < MAX_SERVER ; i ++) {
			...
			int ver = sharedPreferences.getInt("smb-ver" + i, 0);
...
//	}
	...
	// 接続済みSMBバージョンの取得
	public int getSmbVersion(int index){
		if (index == INDEX_LOCAL) {
			return 0;
		}else{
			return mServer[index].getSmbVersion();
		}
	}

	// 接続済みSMBバージョンの設定
	public void setSmbVersion(int ver) {
		if (mSelect == INDEX_LOCAL) {
		}else{
			mServer[mSelect].setSmbVersion( ver );
		}
	}
...
//			return "http://" + mServer[index].getHost();
	...
	public void setData(int index, ServerData data) {
		if (0 <= index && index < MAX_SERVER) {
			...
			mServer[index].setSmbVersion(data.getSmbVersion());
			save(index);
		}
	}
	...
	public void save(int index) {
		...
		ed.putInt( "smb-ver" + index, mServer[index].getSmbVersion() );
		ed.commit();
	}
